...
function addUserImg(address people,string memory headImg) public{
 ...
 //物品上架
 function addGoods(address owner,string memory name,string memory species,uint rent, uint ethPledge,string[] memory goodsImgs) public returns(uint){
  //分类必须存在 
  ...
  for(uint i=0;i<=goodsImgs.length;i++){
    goodsImgData[number].goodsImgs.push(goodsImgs[i]);
  }
 ...
 //删除物品
 function delGoods(uint id) public{
   //必须是借出人才可以修改
    require(goodsData[id].owner ==msg.sender,"owner isn't msg");
...
uint[] public goodsId;
 ...
 //判断物品是否上架
 function isGoodExist(uint id) public view returns(bool){
...
//  借用物品
 ...
 function agreeBack(uint id,uint backs) public{
   ...
   goodsData[id].backData[backs].isAgree=true;
 }
 ...
 function backGoods(uint id,uint backs,string memory over) public payable{
   ...
   if(goodsData[id].backData[backs].isAgree==true){
